/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package amc.view.doctor;

import amc.model.entity.User;
import java.io.BufferedReader;
import java.io.BufferedWriter;
import java.io.FileReader;
import java.io.FileWriter;
import java.io.IOException;
import javax.swing.JTable;
import javax.swing.table.DefaultTableModel;

/**
 *
 * @author Administrator
 */
public class Appointment {
    private final String doctorId;
    private String filepath = "C:\\Users\\Administrator\\Documents\\NetBeansProjects\\Assignment\\APU-Medical-Centre\\database\\Appointment.txt";
        
    public Appointment(User currentUser) {
        this.doctorId = currentUser.getUserId();
    }
    
//    public void make_appointment(String customer_ID, String customer_Name, LocalDate appointment_Date, String appointment_Time, String appointment_Reason){
//            try(BufferedWriter writer = new BufferedWriter(new FileWriter(filepath, true))){
//                this.Appointment_ID = "APT001";
//                this.Customer_ID = customer_ID;
//                this.Customer_Name = customer_Name;
//                this.appointmentDate = appointment_Date;
//                    this.appointmentTime = appointment_Time;
//                    this.appointmentReason = appointment_Reason;
//                    String aptdetail = Appointment_ID + "," +
//                    Customer_ID + "," +
//                    Customer_Name + "," +
//                    Doctor_ID + "," +
//                    doctorName + "," +
//                    departmentID + "," +
//                    departmentName + "," +
//                    staffID + "," +
//                    staffName + "," +
//                    appointmentDate + "," +
//                    appointmentTime + "," +
//                    services + "," +
//                    appointmentReason + "," +
//                    appointmentFeedback + "," +
//                    prescription + "," +
//                    appointmentStatus;
//                    writer.append(aptdetail);
//                    writer.newLine();
//                    System.out.println(aptdetail);
//                }
//            
//                catch(IOException e){
//                    System.out.println("Error writing");
//                }
//            }
    
    public void view_appointment(JTable tableName, String status) {
        boolean view_current = status.equals("current");
        DefaultTableModel model = new DefaultTableModel();
        model.setColumnIdentifiers(new String[] {
            "Appointment_ID", "Customer_ID", "Doctor_ID", "Staff_ID", 
            "Department_ID", "Date", "Time", "Status", "Doctor_Feedback"
        });

        try (BufferedReader reader = new BufferedReader(new FileReader(filepath))) {
            String line;
            while ((line = reader.readLine()) != null) {
                String[] rows = line.split("\\|");
                
                
                
                if (rows.length == 9) {
                    String statusCode = rows[7].trim();
                    String rowDoctorId = rows[2].trim();
//                    if (rowDoctorId.equals(doctorId)){
                        if ((rowDoctorId.equals(doctorId)) && ((view_current && statusCode.equals("0")) ||
                            (!view_current && !statusCode.equals("0")))) {
                            model.addRow(rows);
                        }
//                    }
                }
            }
        } catch (IOException e) {
            System.out.println("Error reading file");
        }
        tableName.setModel(model);
    }
}
